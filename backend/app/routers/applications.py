...
from app.models.application import ApplicationCreate, ApplicationDataUpdate, CommentCreate
...
def create_application(body: ApplicationCreate, current_user: dict = Depends(get_current_user)):
    ...
    app = apps_db.create_application(current_user["email"], body.window_id, body.scholarship_type)
    return app
...
def save_application_data(app_id: str, body: ApplicationDataUpdate, current_user: dict = Depends(get_current_user)):
    app = apps_db.get_application(app_id)
    if not app:
        raise HTTPException(status_code=404, detail="Application not found")
...
@router.post("/{app_id}/comments", dependencies=[Depends(require_reviewer_or_admin)], status_code=201)
def add_comment(app_id: str, body: CommentCreate, current_user: dict = Depends(get_current_user)):
    app = apps_db.get_application(app_id)
    if not app:
        raise HTTPException(status_code=404, detail="Application not found")
